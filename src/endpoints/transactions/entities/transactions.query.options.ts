...
export class TransactionQueryOptions {
  ...
  private static readonly ACTION_TRANSFER_VALUE_MAX_SIZE: number = 50;
  ...
  static applyDefaultOptions(size: number, options: TransactionQueryOptions): TransactionQueryOptions {
    ...
    if (options.withActionTransferValue === true && size > TransactionQueryOptions.ACTION_TRANSFER_VALUE_MAX_SIZE) {
      throw new BadRequestException(`'withActionTransferValue' flag can only be activated for a maximum size of ${TransactionQueryOptions.ACTION_TRANSFER_VALUE_MAX_SIZE}`);
    }

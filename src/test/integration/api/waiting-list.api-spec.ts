...
describe("API Testing", () => {
  ...
  describe('/waiting-list', () => {
    it('should check waiting-list status response code', async () => {
      const checker = new ApiChecker('waiting-list', app.getHttpServer());
      await checker.checkStatus();
    });

    it('should check response body for waiting-list', async () => {
      const checker = new ApiChecker('waiting-list', app.getHttpServer());
      await expect(checker.checkArrayResponseBody()).resolves.not.toThrowError('Invalid response body!');
    });

    it('should check waiting-list pagination', async () => {
      const checker = new ApiChecker('waiting-list', app.getHttpServer());
  ...
  describe('/waiting-list/count', () => {
    it('should return alternative count of waiting-list', async () => {
      const checker = new ApiChecker(`waiting-list`, app.getHttpServer());

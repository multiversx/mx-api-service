...
describe('NetworkService', () => {
  ...
  describe('getEconomics', () => {
    it('should return cached value when available', async () => {
      ...
      const getOrSetCacheSpy = jest.spyOn(networkService['cachingService'], 'getOrSetCache').mockImplementation((_key, getter) => getter());

      jest.spyOn(networkService['cachingService'], 'getOrSetCache').mockImplementationOnce((_key) => Promise.resolve(expectedEconomics));

      const result = await networkService.getEconomics();

      expect(getOrSetCacheSpy).toHaveBeenCalledWith(CacheInfo.Economics.key, expect.any(Function), CacheInfo.Economics.ttl);
  ...
  describe('getAbout', () => {
    it('should return API general information', async () => {
      ...
      jest
        .spyOn(networkService['cachingService'], 'getOrSetCache')
        // eslint-disable-next-line require-await
